...
В этом репозитории находится &laquo;Экспериментальный корпус белорусского языка&raquo;, разработка которого была начата в 2012 году на филологическом факультете БГУ Оксаной Волчек и Владиславом Порицким. В версии от 5 января 2014 г. объём корпуса составляет свыше 7.5 млн токенов газетных и художественных текстов. Морфологическая разметка нанесена с помощью [лемматизатора ЭКБЯ](https://github.com/poritski/YABC_Tagger), омонимия не снята. Корпус оснащён мощной поисковой программой для оффлайнового использования.
...
Чтобы подготовить поисковик к работе, запустите скрипт `MAKE.bat` (или его адаптацию под Unix shell). Скрипт сцепляет один из подкорпусов, по техническим причинам разбитый на две части, и собирает вспомогательные файлы-индексы. Индексация может занять несколько минут.
...
<tr><td>&laquo;Звязда&raquo;</td><td>3367004</td></tr>
...
<tr><td>Всего</td><td>7761146</td></tr>
</table>
</center>
...
Результаты поиска записываются в файл `results.txt`, который имеет формат TSV с тремя полями:

*  идентификатор запроса;
*  найденный контекст;
*  подробная информация о тексте-источнике.
...
Результаты поиска записываются в файл `results.txt`, который имеет формат TSV с пятью полями:

*  имя файла, содержащего текст-источник;
*  идентификатор запроса;
*  левый контекст;
*  сам найденный токен;
*  правый контекст.
...
_28 ноября 2013 г.:_ Первый полный релиз. Тексты основного корпуса лемматизованы, доступен поиск по неоднословным конструкциям с учётом лемм и грамматических ярлыков.

_5 января 2014 г.:_ Добавлено свыше тысячи статей из газеты &laquo;Звязда&raquo;.